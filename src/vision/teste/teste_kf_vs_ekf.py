...
class TestKFvsEKF2D:
    ...
    def test_2d_tracking_accuracy(self, filters_2d, trajectory_2d):
        ...
        for i, (true_state, measurement) in enumerate(zip(true_trajectory, measurements)):
            # Filtro de Kalman
            kf.predict(dt)
        ...
        print(f"  Improvement:        {(1 - ekf_mse/kf_mse)*100:.2f}%")
    ...
    def test_2d_velocity_tracking(self, filters_2d, trajectory_2d):
        ...
        print(f"  Improvement:        {(1 - ekf_vel_mse/kf_vel_mse)*100:.2f}%")
    ...
    def test_2d_processing_time(self, filters_2d, trajectory_2d):
        ...
        for measurement in measurements:
            kf.predict(dt)
            kf.update(measurement)
        kf_time = (time.perf_counter() - start) / len(measurements) * 1e6  # microsegundos
    ...
    def test_2d_noise_robustness(self, filters_2d):
        ...
        for noise_level, res in results.items():
            improvement = (1 - res['ekf_mse']/res['kf_mse'])*100
            print(f"  Noise {noise_level}: KF MSE={res['kf_mse']:.6f}, "
                  f"EKF MSE={res['ekf_mse']:.6f}, improvement={improvement:.2f}%")
...
class TestKFvsEKF1D:
    ...
    def test_1d_angle_tracking(self, filters_1d, trajectory_1d):
        ...
        for true_state, measurement in zip(true_trajectory, measurements):
            ...
            kf.update(measurement)
            kf_state = np.array(kf.x).flatten()
            # Calcular erro de ângulo (considerando wrap)
        ...
        print(f"  Improvement:     {(1 - ekf_mae/kf_mae)*100:.2f}%")
    ...
    def test_1d_angular_velocity_tracking(self, filters_1d, trajectory_1d):
        ...
        for true_state, measurement in zip(true_trajectory, measurements):
            ...
            kf.update(measurement)
            kf_state = np.array(kf.x).flatten()
            kf_vel_error = abs(kf_state[1] - true_state[1])
            kf_vel_errors.append(kf_vel_error)
            
            ekf.predict(dt)
            ekf.update(measurement)
            ekf_state = np.array(ekf.x).flatten()
            ekf_vel_error = abs(ekf_state[1] - true_state[1])
            ekf_vel_errors.append(ekf_vel_error)
        ...
        print(f"  Improvement:     {(1 - ekf_vel_mae/kf_vel_mae)*100:.2f}%")
    ...
    def test_friction_impact_2d(self):
        ...
        for friction in friction_values:
            ...
            for step in range(n_steps):
                # Trajetória com atrito exponencial real
                t = step * dt
    ...
    def test_friction_impact_1d(self):
        ...
        for friction in friction_values:
            ...
            for step in range(n_steps):
                ...
                ekf.update(z)
                ekf_state = np.array(ekf.x).flatten()
                angle_diff = ekf_state[0] - theta_true
                angle_diff = (angle_diff + np.pi) % (2 * np.pi) - np.pi
                error = abs(angle_diff)
                errors.append(error)
...
def test_summary():
    """Teste que executa todos os testes e imprime um resumo."""
    print("\n" + "="*70)
