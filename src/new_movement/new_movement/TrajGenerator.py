...
class TrajectoryGenerator():
    def __init__(self, constrainsts: Optional[MoveConstraints] = None):
        ...
        if constrainsts is not None:
            ...
            self.constrainsts = MoveConstraints(DEFAULT_VELOCITY_CONSTRAINST, DEFAULT_ACCELERATION_CONSTRAINST)

    def generate(self, curPos: Vector2D,
                       curVel: Vector2D,
                       tarState: State) -> TrajectorySegment:
        ''' Generates a piecewise constant acceleration motion path using the BB_Steer '''

        bb_output = bb_steer(curPos + curVel,
        ...
        motion_path = MotionPath([MotionPrimitive(Vector2D(out[0][0], out[0][1]), out[1]) for out in bb_output])
