...
class TrajectorySegment:
    ...
    def add_child(self, child: "TrajectorySegment") -> None:
        ...
        if(local_destination.position.distance(child.initPos) < 1e-8 and \
           local_destination.velocity.distance(child.initVel) < 1e-8):
            self.child = child
        else:
...
class Trajectory:
    ...
    def to_list(self, time_step: Optional[float] = None,
                      samples_size: Optional[int] = None,
                      output_states: bool = False) -> List[Vector2D] | List[State]:
        ...
        if (time_step is not None and samples_size is not None) or \
           (time_step is None and samples_size is None):
            raise ValueError("to_list should have defined either time_steps or samples_size, but not both or neither.")
        ...
        while(cur_time <= total_time):
            traj_list.append(self.get_state(cur_time) if output_states else self.get_position(cur_time))
            cur_time += time_step
        ...
        if(total_time % time_step < 1e-8):
            traj_list.append(self.get_destination() if output_states else self.get_destination().position)
