...
from movement.local_planner import ObstacleFactory
from movement.entities.obstacle import GenericCircleObstacle, EnemyRobotObstacle
...
class TestRobotCollections:
    """
    The ally loop used to index the robot list with the message it had just iterated,
    which cost the robot its plan on every cycle. Every other test here passes
    ally_robots=[], so the loop never ran and the bug reached the field instead.
    """
    ...
    def test_allies_become_obstacles_and_exclude_the_planning_robot(self):
        obstacles = self._obstacles(self._robots(1, 2, 3), [], robot_id=1)

        assert len(obstacles) == 2
        assert all(isinstance(o, EnemyRobotObstacle) for o in obstacles)

    def test_enemies_become_obstacles(self):
        assert len(self._obstacles([], self._robots(4, 5))) == 2

    def test_both_collections_together(self):
        obstacles = self._obstacles(self._robots(1, 2), self._robots(6, 7))
