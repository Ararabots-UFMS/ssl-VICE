...
import math
class DefendAreaRightDown(LeafNode):
    ...
    def __init__(self, name, robot_id: int, robot_x: float, robot_y:float, ball_x: float, ball_y: float, goal_x: float = -2250, goal_y: float = 0, robot_radius: float = 85, distance = 5):
        super().__init__(name)
        ...
        distance_robots = 2 * robot_radius + distance
        dist_ball = (4000 - ball_x)/(goal_x - ball_x)
        target_x = 4000 + -robot_radius * (goal_x - ball_x) - distance_robots * (-(goal_y - ball_y)) / math.sqrt((goal_x - ball_x) ** 2 + ((goal_y - ball_y) ** 2))
        target_y = ball_y + dist_ball * (goal_y - ball_y) + -robot_radius * (goal_y - ball_y) - distance_robots * (-(goal_x - ball_x)) / math.sqrt((goal_x - ball_x) ** 2 + ((goal_y - ball_y) ** 2))
