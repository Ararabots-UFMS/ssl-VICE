...
class CheckState(Behaviour):
    ...
    def update(self):
        ...
        self.logger.debug(
            f"String is '{self.blackboard.referee.command}', does not match '{self.name}', returning FAILURE"
        )
...
class CheckKickoffTeam(Behaviour):
    def __init__(self, name, commmand, blackboard):
        super(CheckState, self).__init__(name)
        self.blackboard = blackboard
        self.commands = commmands
    ...
    def update(self):
        self.logger.debug(f"CheckState::update {self.name}")
        # Condição: Se a string de entrada for igual ao nome do comportamento (subárvore)
        if self.blackboard.gui.command in self.commands:
            self.logger.debug(f"String matches '{self.name}', returning SUCCESS")
            return Status.SUCCESS
        self.logger.debug(
            f"String is '{self.blackboard.referee.command}', does not match '{self.name}', returning FAILURE"
